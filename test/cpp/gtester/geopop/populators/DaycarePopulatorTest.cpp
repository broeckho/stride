...
protected:
        ...
        const unsigned int     m_ppc = GeoGridConfig{}.pools.pools_per_daycare;
};
    ...
    TEST_F(DaycarePopulatorTest, OneLocationTest)
    {
        ...
        auto& k12Pools = location->RefPools(Id::Daycare);

        ASSERT_EQ(k12Pools.size(), 5 * m_geogrid_config.pools.pools_per_daycare);
    ...
    TEST_F(DaycarePopulatorTest, TwoLocationTest)
    {
        ...
        EXPECT_EQ(k12Pools1.size(), 3 * m_geogrid_config.pools.pools_per_daycare);
        EXPECT_EQ(k12Pools2.size(), 3 * m_geogrid_config.pools.pools_per_daycare);
        EXPECT_EQ(k12Pools3.size(), 3 * m_geogrid_config.pools.pools_per_daycare);
