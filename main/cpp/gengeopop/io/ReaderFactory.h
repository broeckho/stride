...
public:
        ...
        std::shared_ptr<CitiesReader> CreateCitiesReader(const filesystem::path& path);
        ...
        std::shared_ptr<CommutesReader> CreateCommutesReader(const filesystem::path& path);
        ...
        std::shared_ptr<HouseholdReader> CreateHouseholdReader(const filesystem::path& path);

private:
        /// Create an istream based on the provided Boost Filesystem Path
        std::unique_ptr<std::istream> OpenFile(const filesystem::path& path) const;
};
} // namespace gengeopop
