...
ostream& operator<<(ostream& out, const GeoGridConfig& config)
{
        ...
        out << setw(w) << "Participation fraction of workplace:" << config.input.particpation_workplace << "\n";
        out << setw(w) << "Target population size" << intToDottedString(config.input.pop_size) << "\n"
            << "\n";
        out << "Calculated:"
            << "\n";
        out << setw(w) << "Daycare toddler count:" << intToDottedString(config.popInfo.popcount_daycare) << "\n";
